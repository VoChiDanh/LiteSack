...
//        }
     ...
     */
    public ItemBuilder damage(int damage) throws IllegalArgumentException {
        if (damage <= 0) {
     ...
     */
    public ItemBuilder enchant(Enchantment enchantment, int level) throws IllegalArgumentException {
        Validate.notNull(enchantment, "The enchantment must not be null");
        if (level <= 0) {
    ...
    public ItemStack build() {
        ...
        if (this.glow) {
            ...
            if (!this.itemMeta.getItemFlags().isEmpty()) this.itemMeta.getItemFlags().clear();
